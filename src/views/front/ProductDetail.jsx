        ...
        <section className="productDetail py-2 py-md-5 mb-5">
            <div className="container">
                <div className="breadcrumb">
                    ...
                    <span className="material-icons-outlined">chevron_right</span>
                    <span className="breadLink active">{tempProduct.title}</span>
                </div>
                <div className="d-flex flex-column flex-md-row gap-4">
                    <div>
                        <div className="productDetail-gallery">
                            ...
                            {tempProduct?.imagesUrl?.map((img) => (
                                <SwiperSlide key={img}>
                                <img src={img} className="main-image" alt={tempProduct.title} />
                                </SwiperSlide>
                            ))}
                            </Swiper>
                            <Swiper
                            ...
                            >
                                ...
                                {tempProduct?.imagesUrl?.map((img) => (
                                    <SwiperSlide key={img}>
                                        <img src={img} className="thumb-image" alt={tempProduct.title} />
                                    </SwiperSlide>
                                ))}
                            </Swiper>
                        </div>
                    ...
                    <div className="d-flex flex-fill flex-column gap-3 gap-md-4">
                        <div className="gap-2 gap-lg-4 align-content-center">
                            <h1 className="h5 m-0 align-self-lg-center">{tempProduct.title}</h1>
                            <p className="fs-6 text-secondary mt-2">{tempProduct.description}</p>
                            <div className="tagBox">
                                {tempProduct.is_newest && (
                                <span className="tag tag-newest">
                                    <span className="material-icons fs-5">verified</span>
                                    <span className="fs-7">新品</span>
                                </span>
                                )}
                                {tempProduct.is_hottest && (
                                <span className="tag tag-hottest">
                                    <span className="material-icons fs-5 text-white">local_fire_department</span>
                                    <span className="fs-7 text-white">TOP</span>
                                </span>
                                )}
                                {tempProduct.origin_price > tempProduct.price && (
                                <span className="tag tag-cheaper">
                                    <span className="material-icons fs-5 text-white">timer</span>
                                    <span className="fs-7 text-white">限時優惠</span>
                                </span>
                                )}
                            </div>
                        </div>
                    <div>
                        ...
                        {tempProduct.origin_price > tempProduct.price && (
                        <del className="fs-5 text-secondary ms-3">
                    ...
                    <div className="d-flex">
                        ...
                        return (
                            <div className="fs-5 form-check ps-0 me-3 checkedStyle" key={color.colorName}>
                                <input
                                    type="radio"
                                    id={`color-${index + 1}`}
                    ...
                    <div className="d-flex gap-md-4 gap-3">
                        <button
                            type="button"
                            disabled={isLoading}
                            className="btn addCart-btn flex-fill d-flex align-items-center justify-content-center gap-2"
                        ...
                        <button
                            type="button"
                            disabled={isLoading}
                            className="btn buy-btn flex-fill d-flex align-items-center justify-content-center gap-2"
                ...
                <div className="pt-0 pt-md-4">
                    ...
                    <Swiper
                        ...
                        loop={hottestProducts.length >= 5} // 如果產品數量大於等於5，則循環播放
                        breakpoints={{
                            // N px 以上 一次顯示幾張
                            640: { slidesPerView: 2 },
                            768: { slidesPerView: 3 },
